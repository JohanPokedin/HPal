...
A[3] = function(icon, isMulti)
	...
	local MagicNPCID = {
		...
		167113, -- Spinemaw Acid Gullet
		167117, -- Spinemaw Larva
		164517, -- Tred'ova
		168578, -- Fungalmancer
		169696, -- Mire Soldier
		168572, -- Fungal Stormer
    ...
    local function DamageRotation(unitID)
        ...
        if A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp" or inCombat then
			...
			if Unit(unitID):HasDeBuffs("BreakAble") == 0 then
				...
				if A.ShieldoftheRighteous:IsReady(player) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):GetRange() <= 5 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and (HealingEngine.GetBelowHealthPercentUnits(90, 40) < 1 or TeamCache.Friendly.Size <= 2) then
				...
				if A.HolyShock:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and GetToggle(2, "HolyShockDPS") and Unit(unitID):GetRange() <= 40 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 then
					return A.HolyShock:Show(icon)
				end
				
				if A.CrusaderStrike:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and A.CrusaderStrike:IsInRange(unitID) and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 then
    ...
    local function HealingRotation(unitID) 
		...
		if GetToggle(1, "StopCast") then
			...
			if inCombat and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and (((Unit(player):IsCastingRemains(A.HolyLight.ID) > Player:Execute_Time(A.HolyShock.ID) or Unit(player):IsCastingRemains(A.FlashofLight.ID) > Player:Execute_Time(A.HolyShock.ID)) and A.HolyShock:IsReadyByPassCastGCD(unitID) and Unit(unitID):HealthDeficit() >= (HolyShockHP * StatMultiplier * AvengingWrathMultiplier))
				or ((Unit(player):IsCastingRemains(A.HolyLight.ID) > Player:Execute_Time(A.WordofGlory.ID) or Unit(player):IsCastingRemains(A.FlashofLight.ID) > Player:Execute_Time(A.WordofGlory.ID)) and A.WordofGlory:IsReadyByPassCastGCD(unitID) and Unit(unitID):HealthDeficit() >= (WordofGloryHP * StatMultiplier * AvengingWrathMultiplier))) then
				return A:Show(icon, ACTION_CONST_STOPCAST)
			end
		...
		for i = 1, #getmembersAll do 
			if combatTime > 0 and useShields and Unit(getmembersAll[i].Unit):GetRange() <= 40 and A.InstanceInfo.KeyStone >= 15 then 
				if not Unit(getmembersAll[i].Unit):IsPet() and not Unit(getmembersAll[i].Unit):IsDead() and A.BlessingofProtection:IsReadyByPassCastGCD(getmembersAll[i].Unit) 
		...
		local _,_,_,_,_,NPCID = Unit(unitID):InfoGUID()
		
		--Blessing of Protection
				
        if A.BlessingofProtection:IsReadyByPassCastGCD(unitID) and NPCID ~= MagicNPCID and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and useShields and combatTime > 0 and not Unit(unitID):IsPet() and not Unit(unitID):IsTank() and not UnitIsUnit(unitID, player) and not Unit(unitID):IsDead() 
        ...
        if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < 0.8 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and Unit(player):HasBuffs(A.DivineFavor.ID, true) > 0 and Unit(unitID):HealthDeficit() >= ((WordofGloryHP/1.211) * 1.3 * AvengingWrathMultiplier * StatMultiplier * 2) and not Unit(unitID):IsDead() and Unit(player):HasBuffs(A.InfusionofLightBuff.ID, true) > 2.3 + GetCurrentGCD() and Unit(unitID):HealthPercent() <= HolyLightHP and Unit(player):GetCurrentSpeed() == 0 then
            return A.HolyLight:Show(icon)
        end    
        
        --Holy Light Divine Favor w/o Infusion
        if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < 0.8 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and Unit(player):HasBuffs(A.DivineFavor.ID, true) > 0 and Unit(unitID):HealthDeficit() >= ((WordofGloryHP/1.211) * AvengingWrathMultiplier * StatMultiplier * 2) and not Unit(unitID):IsDead() and Unit(unitID):HealthPercent() <= HolyLightHP and Unit(player):GetCurrentSpeed() == 0 then
            return A.HolyLight:Show(icon)
        end    
		...
		if not A.BeaconofVirtue:IsTalentLearned() and A.BeaconofFaith:IsTalentLearned() and BeaconWorkMode == "Beacon of Faith" then
            if TeamCacheFriendlyType == "party" and (combatTime == 0 or Unit("player"):HasBuffs(340880) > 0) and A.BeaconofLight:IsReady(unitID) and Unit(unitID):Role("TANK") and Unit(unitID):HasBuffs(A.BeaconofFaith.ID, true) == 0 and Unit(unitID):HasBuffs(A.BeaconofLight.ID, true) == 0 then
                return A.BeaconofLight:Show(icon)
			elseif TeamCacheFriendlyType == "raid" and combatTime == 0 and A.BeaconofLight:IsReady(unitID) and Unit(unitID):Role("TANK") and Unit(unitID):HasBuffs(A.BeaconofFaith.ID, true) == 0 and Unit(unitID):HasBuffs(A.BeaconofLight.ID, true) == 0 and HealingEngine.GetBuffsCount(A.BeaconofLight.ID, 0, true) == 0 then
                return A.BeaconofLight:Show(icon)
            end
			if TeamCacheFriendlyType ~= "none" and combatTime == 0 and A.BeaconofFaith:IsReady(unitID) and not Unit(unitID):Role("TANK") and Unit(unitID):HasBuffs(A.BeaconofLight.ID, true) == 0 and Unit(unitID):HasBuffs(A.BeaconofFaith.ID, true) == 0 and HealingEngine.GetBuffsCount(A.BeaconofFaith.ID, 0, true) == 0 then
				return A.BeaconofFaith:Show(icon)
			end
			
			if combatTime > 0 and NPCID == 173729 then
		    for i = 1, #getmembersAll do 
                if Unit(getmembersAll[i].Unit):GetRange() <= 60 then 
                    	if A.BeaconofLight:IsReady(getmembersAll[i].Unit) and not Unit(getmembersAll[i].Unit):HasSpec(63) and not Unit(getmembersAll[i].Unit):HasSpec(64) and A.BeaconofLight:GetSpellTimeSinceLastCast() > 90 and not Unit(getmembersAll[i].Unit):Role("TANK") and HealingEngine.IsMostlyIncDMG(getmembersAll[i].Unit) and Unit(getmembersAll[i].Unit):HasBuffs(A.BeaconofFaith.ID, true) == 0 and Unit(getmembersAll[i].Unit):HasBuffs(A.BeaconofLight.ID, true) == 0 then
							HealingEngine.SetTarget(getmembersAll[i].Unit, 0.3)
							return A.BeaconofLight:Show(icon)                   
						end                    
                end                
        ...
        --Beacon of Virtue
        if A.BeaconofVirtue:IsReadyByPassCastGCD(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and A.BeaconofVirtue:IsTalentLearned() and TeamCacheFriendlyType ~= "none" and
        (
            (       
                not IsUnitEnemy(target) and
        ...
        -- #17 HPvE DivineToll
        if A.DivineToll:IsReadyByPassCastGCD(unit) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and combatTime > 0 then
            for i = 1, #getmembersAll do 
        ...
        if not Action.InstanceInfo.isRated or Action.InstanceInfo.isRated and (Unit(unitID):HealthPercent() >= 30 or Unit(unitID):HealthPercent() < 30 and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady()) then
				...
				for i = 1, 40 do
					for x = 1, #IsCCDebuff do
						...
						if debuffType == IsCCDebuff[x] and Unit(IsCCDebuff[i].Unit):HasDeBuffs(342938) == 0 and Unit(IsCCDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 and AuraIsValid(IsCCDebuff[i].Unit, "UseDispel", "Dispel") then
							HealingEngine.SetTarget(IsCCDebuff[i].Unit, 0.3)
						end
					end
					for x = 1, #IsSlowDebuff do
                        local name, rank, icon, count, debuffType = UnitDebuff(unitID, i) 
                        if debuffType == IsSlowDebuff[x] and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(342938) == 0 and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 and AuraIsValid(IsSlowDebuff[i].Unit, "UseDispel", "Dispel") and Unit(IsSlowDebuff[i].Unit):HasSpec(MELEE) then
                            HealingEngine.SetTarget(IsSlowDebuff[i].Unit, 0.3)
                        end
                    end
					for x = 1, #IsSlowDebuff do
						local name, rank, icon, count, debuffType = UnitDebuff(unitID, i) 
					...
					for x = 1, #IsDoTDebuff do					
						...
						if debuffType == IsDoTDebuff[x] and Unit(IsDoTDebuff[i].Unit):HasDeBuffs(342938) == 0 and AuraIsValid(IsDoTDebuff[i].Unit, "UseDispel", "Dispel") and Unit(IsDoTDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then
							HealingEngine.SetTarget(IsDoTDebuff[i].Unit, 0.3)
						end
					end
				end
			end
			if (A.Zone == "arena" or A.InstanceInfo.isRated) and A.BlessingofFreedom:IsReady() and A.GetToggle(2, "DispelSniper") then
				for i = 1, #getmembersAll do 
                    ...
                    and AuraIsValid(getmembersAll[i].Unit, true, "BlessingofFreedom") and Unit(getmembersAll[i].Unit):HasSpec(MELEE) and Unit(getmembersAll[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then  
                        HealingEngine.SetTarget(getmembersAll[i].Unit, 0.3)                                      
                    end                
                end
            end
            ...
            -- #2 HPvE BoF
            if A.BlessingofFreedom:IsReady(unitID) and ((A.Zone == "arena" or A.InstanceInfo.isRated) or Unit(unitID):GetCurrentSpeed() <= 70) and Unit(unitID):HasBuffs(A.BlessingofFreedom.ID) == 0 and
            useUtils and
            (
                -- MouseOver
                (
        ...
        if A.WordofGlory:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (WordofGloryHP * StatMultiplier * AvengingWrathMultiplier) and Player:HolyPower() >= 5 then
            return A.WordofGlory:Show(icon)
        end
        
        if A.HolyPrism:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (HolyPrismHP * StatMultiplier * AvengingWrathMultiplier) then
            return A.HolyPrism:Show(icon)
        end       
        
        --Holy Shock target
        if A.HolyShock:IsReady(unitID) and IsUnitFriendly(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (HolyShockHP * StatMultiplier * AvengingWrathMultiplier) then
            return A.HolyShock:Show(icon)
        end
        
        --Word of Glory 3 HP
        if A.WordofGlory:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (WordofGloryHP * StatMultiplier * AvengingWrathMultiplier) then
            return A.WordofGlory:Show(icon)
        end
        ...
        --Bestow Faith
        if A.BestowFaith:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (BestowFaithHP * StatMultiplier * AvengingWrathMultiplier) then
            return A.BestowFaith:Show(icon)
        end    
        
        if A.DivineFavor:IsTalentLearned() and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and IsUnitFriendly("target") and Unit("target"):HasDeBuffs(A.Cyclone.ID) == 0 and Unit("target"):GetRange() <= 40 and Unit("target"):HealthDeficit() >= ((WordofGloryHP/1.211) * AvengingWrathMultiplier * StatMultiplier * 2) and A.DivineFavor:IsReady(player) then
        ...
        if A.Zone == "pvp" or A.Zone == "arena" or A.InstanceInfo.isRated or inCombat then
			...
			if Unit(unitID):HasDeBuffs("BreakAble") == 0 then
				...
				if A.CrusaderStrike:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and A.CrusaderStrike:IsInRange(unitID) and IsUnitEnemy(unitID) and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 
				and not Unit(target):IsDead() and (A.CrusadersMight:IsTalentLearned() and (Player:HolyPower() == 5 and A.HolyShock:GetCooldown() >= 1.5 or ((Player:HolyPower() < 5 and A.HolyShock:GetCooldown() >= 0.8) or (A.HolyShock:GetCooldown() == 0 and A.CrusaderStrike:GetSpellCharges() == 2 and Player:HolyPower() < 5))) or not A.CrusadersMight:IsTalentLearned() and Player:HolyPower() < 5) then
					return A.CrusaderStrike:Show(icon)        
				end
        ...
        if A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp" or inCombat then
            ...
            if A.ShieldoftheRighteous:IsReady(player) and A.CrusaderStrike:IsInRange(unitID) and IsUnitEnemy(unitID) and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 
			and (HealingEngine.GetBelowHealthPercentUnits(90, 40) < 1 or (HealingEngine.GetBelowHealthPercentUnits(50, 40) < 1 and TeamCache.Friendly.Size <= 2)) then
                return A.ShieldoftheRighteous:Show(icon)
            end
    ...
    local function FillerRotation(unitID) 
        ...
        -- Light of the Martyr
        if A.LightofMartyr:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and (Unit(player):HealthPercent() >= 40 or Unit(player):HasBuffs(A.DivineShield.ID) > 0) and Unit(unitID):HealthPercent() <= LightofMartyrHP and Unit(player):GetCurrentSpeed() ~= 0 and not UnitIsUnit(unitID, player) then
            return A.LightofMartyr:Show(icon)
        end    

		if Unit(player):HasDeBuffs(A.Quake.ID) == 0 then
		
		if not inCombat or (inCombat and (not A.HolyShock:IsReady() or A.HolyShock:IsReady() and Unit(unitID):HealthDeficit() < (HolyShockHP * StatMultiplier * AvengingWrathMultiplier)) and 
			(not A.WordofGlory:IsReady() or A.WordofGlory:IsReady() and Unit(unitID):HealthDeficit() < (WordofGloryHP * StatMultiplier * AvengingWrathMultiplier))) then 
		
			--Flash of Light Infusion proc
			if A.FlashofLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.FlashofLight.ID) and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() and Unit(player):HasBuffs(A.InfusionofLightBuff.ID, true) > 1.4 + A.GetCurrentGCD() and Unit(unitID):HealthPercent() <= FlashofLightHP and Unit(player):GetCurrentSpeed() == 0 then
				return A.FlashofLight:Show(icon)
			end
			
			--Holy Light Infusion proc
			if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.HolyLight.ID) and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() and Unit(player):HasBuffs(A.InfusionofLightBuff.ID, true) > 2.3 + GetCurrentGCD() and Unit(unitID):HealthPercent() <= HolyLightHP and Unit(player):GetCurrentSpeed() == 0 then
				return A.HolyLight:Show(icon)
			end    
			
			--Flash of Light no infusion
			if A.FlashofLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.FlashofLight.ID) and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthPercent() <= FlashofLightHP and Unit(player):GetCurrentSpeed() == 0 then
				return A.FlashofLight:Show(icon)
			end
			
			--Holy Light no infusion
			if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.HolyLight.ID)and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthPercent() <= HolyLightHP and Unit(player):GetCurrentSpeed() == 0 then
				return A.HolyLight:Show(icon)
			end    
