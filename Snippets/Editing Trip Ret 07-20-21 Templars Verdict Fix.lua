--######################################################
--##### Edited TRIP'S RETRIBUTION PALADIN by Johan #####
--######################################################
...
A[3] = function(icon, isMulti)
	...
	local function EnemyRotation(unitID)
		...
		if A.Trinket1:IsReady(unitID) and BurstIsON and A.Trinket1:GetItemCategory() ~= "DEFF" and not Temp.IsSlotTrinketBlocked[A.Trinket1.ID] and A.Trinket1:AbsentImun(unitID, Temp.TotalAndMagPhys) then 
			return A.Trinket1
		end 
		if A.Trinket2:IsReady(unitID) and BurstIsON and A.Trinket2:GetItemCategory() ~= "DEFF" and not Temp.IsSlotTrinketBlocked[A.Trinket2.ID] and A.Trinket2:AbsentImun(unitID, Temp.TotalAndMagPhys) then 
			return A.Trinket2
		end 
    ...
    local function FriendlyRotation(unitID)
		...
		if not inCombat and A.Redemption:IsReady(unitID) and Unit(unitID):IsPlayer() and Unit(unitID):IsDead() then
