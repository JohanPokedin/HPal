...
A[3] = function(icon, isMulti)
    ...
    local function HealingRotation(unitID) 
        ...
        if A.BlessingofProtection:IsReady(unitID) and NPCID ~= MagicNPCID and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and useShields and combatTime > 0 and not Unit(unitID):IsPet() and not Unit(unitID):IsTank() and not UnitIsUnit(unitID, player) and not Unit(unitID):IsDead() 
