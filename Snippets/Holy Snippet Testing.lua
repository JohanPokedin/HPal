...
TMW:RegisterCallback("TMW_ACTION_HEALINGENGINE_UNIT_UPDATE", function(callbackEvent, thisUnit, db, QueueOrder) 
        ...
        if Unit(thisUnit.Unit):GetRange() <= MinRange then
            MasteryEffect = 1 + (Player:MasteryPct()/100)
        end
        if Unit(thisUnit.Unit):GetRange() > MinRange then
            MasteryEffect = 1 + ((Player:MasteryPct() - (Player:MasteryPct()/MaxRange * (Unit(thisUnit.Unit):GetRange()-MinRange)))/100)
        end
		...
		if Unit(thisUnit.Unit):HasDeBuffs(355714) > 0 then
			ExecutionerMultiplier = 0.5
		elseif Unit(thisUnit.Unit):HasDeBuffs(355714) == 0 then
			ExecutionerMultiplier = 1
		end
		local HealingMultiplier = StatMultiplier * AvengingWrathMultiplier * ExecutionerMultiplier
        ...
        if IsUnitEnemy("target") or NPCID == 171577 or NPCID == 165759 then
			...
			(
				(thisUnit.MHP - thisUnit.realAHP  < WordofGloryHP * HealingMultiplier and A.WordofGlory:IsReady() or not A.WordofGlory:IsReady()) and 
				(thisUnit.MHP - thisUnit.realAHP < HolyShockHP * HealingMultiplier and A.HolyShock:IsReady() or not A.HolyShock:IsReady()) and
				(not A.BestowFaith:IsTalentLearned() or (A.BestowFaith:IsTalentLearned() and (thisUnit.MHP - thisUnit.realAHP < BestowFaithHP * HealingMultiplier and A.BestowFaith:IsReady() or not A.BestowFaith:IsReady()))) and
				(not A.HolyPrism:IsTalentLearned() or (A.HolyPrism:IsTalentLearned() and (thisUnit.MHP - thisUnit.realAHP < HolyPrismHP * HealingMultiplier and A.HolyPrism:IsReady() or not A.HolyPrism:IsReady()))) and
				(thisUnit.MHP - thisUnit.realAHP < LightofMartyrHP * HealingMultiplier * MaraadMultiplier or not A.LightofMartyr:IsReady()) and
				(thisUnit.MHP - thisUnit.realAHP < HolyLightHP * HealingMultiplier or Unit(player):GetCurrentSpeed() > 0 or not A.HolyLight:IsReady()) and
				(thisUnit.MHP - thisUnit.realAHP < FlashofLightHP * HealingMultiplier or Unit(player):GetCurrentSpeed() > 0 or not A.FlashofLight:IsReady()) and
				(not A.DivineToll:IsReady() or A.DivineToll:IsReady() and HealingEngine:GetBelowHealthPercentUnits(GetToggle(2, "DivineTollHPParty"), 30) < GetToggle(2, "DivineTollUnitsParty")) and
				(not UseLightofDawn or (UseLightofDawn and (not A.LightofDawn:IsReady() or A.LightofDawn:IsReady() and (HealingEngine.GetBelowHealthPercentUnits(LightofDawnHP, LightofDawnRange) < LightofDawnUnits))))
			) 
			
			and (not AuraIsValid(thisUnit.Unit, "UseDispel", "Dispel") or AuraIsValid(thisUnit.Unit, "UseDispel", "Dispel") and not A.Cleanse:IsReady()) 
			and (not AuraIsValid(thisUnit.Unit, true, "BlessingofFreedom") or AuraIsValid(thisUnit.Unit, true, "BlessingofFreedom") and not A.BlessingofFreedom:IsReady()) then
...
A[3] = function(icon, isMulti)
    ...
    local MagicNPCID = {
		...
		326617,	326632,	327481,	327648,	328180,	328331,	328664,	329110,	329325,	329326,	329608,	329862,	329905,	332605,	332707,	333708,	334505,	334765,	338353,	338729,	340026,	342207, 347283, 348756, 349954, 350713, 
    ...
    local function HealingRotation(unitID) 
        ...
        if GetToggle(1, "StopCast") then
            ...
            if inCombat and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and (((Unit(player):IsCastingRemains(A.HolyLight.ID) > Player:Execute_Time(A.HolyLight.ID)/2 or Unit(player):IsCastingRemains(A.FlashofLight.ID) > Player:Execute_Time(A.FlashofLight.ID)/2) and A.HolyShock:IsReady(unitID) and Unit(unitID):HealthDeficit() >= (HolyShockHP * HealingMultiplier))
                or ((Unit(player):IsCastingRemains(A.HolyLight.ID) > 0.5 or Unit(player):IsCastingRemains(A.FlashofLight.ID) > 0.5) and A.WordofGlory:IsReady(unitID) and Unit(unitID):HealthDeficit() >= (WordofGloryHP * HealingMultiplier)) or (Unit(player):IsCastingRemains(A.HolyLight.ID) > 0 and Unit(unitID):HealthDeficit() < (HolyLightHP * HealingMultiplier)) or (Unit(player):IsCastingRemains(A.FlashofLight.ID) > 0 and Unit(unitID):HealthDeficit() < (FlashofLightHP * HealingMultiplier))) then
                return A:Show(icon, ACTION_CONST_STOPCAST)
            end
        ...
        if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < 0.8 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and Unit(player):HasBuffs(A.DivineFavor.ID, true) > 0 and Unit(unitID):HealthDeficit() >= ((WordofGloryHP/1.211) * HealingMultiplier * 1.5) and not Unit(unitID):IsDead() and Unit(unitID):HealthPercent() <= HolyLightHP and Unit(player):GetCurrentSpeed() == 0 then
		...
		local PVEDispelRemaining, PVEDispelTotal = Unit(unitID):HasDeBuffs(PVEDispelDebuffs)
		local PVEBOFRemaining, PVEBOFTotal = Unit(unitID):HasDeBuffs(PVEBOFDebuffs)
		local PVPDispelRemaining, PVPDispelTotal = Unit(unitID):HasDeBuffs(PVPDispelDebuffs)
		local PVPBOFRemaining, PVPBOFTotal = Unit(unitID):HasDeBuffs(PVPBOFDebuffs)
		local CCRemaining, CCTotal = Unit(unitID):HasDeBuffs(IsCCDebuff)
		local SlowRemaining, SlowTotal = Unit(unitID):HasDeBuffs(IsSlowDebuff)
		local DoTRemaining, DoTTotal = Unit(unitID):HasDeBuffs(IsDoTDebuff)
        ...
        if not Action.InstanceInfo.isRated or Action.InstanceInfo.isRated and (Unit(unitID):HealthPercent() >= 30 or Unit(unitID):HealthPercent() < 30 and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady()) then
            ...
            if (A.Zone == "arena" or A.InstanceInfo.isRated) and A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, 40 do
                    for x = 1, #IsCCDebuff do
                        ...
                        if debuffType == IsCCDebuff[x] and Unit(IsCCDebuff[i].Unit):HasDeBuffs(342938) == 0 and CCTotal - CCRemaining >= 0.5 and Unit(IsCCDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 and AuraIsValid(IsCCDebuff[i].Unit, "UseDispel", "Dispel") then
                            HealingEngine.SetTarget(IsCCDebuff[i].Unit, 0.5)
                        end
                    end
                    for x = 1, #IsSlowDebuff do
                        local name, rank, icon, count, debuffType = UnitDebuff(unitID, i) 
                        if debuffType == IsSlowDebuff[x] and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(342938) == 0 and SlowTotal - SlowRemaining >= 0.5 and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 and AuraIsValid(IsSlowDebuff[i].Unit, "UseDispel", "Dispel") and Unit(IsSlowDebuff[i].Unit):HasSpec(PVPMELEE) then
                            HealingEngine.SetTarget(IsSlowDebuff[i].Unit, 0.5)
                        end
                    end
                    for x = 1, #IsSlowDebuff do
                        local name, rank, icon, count, debuffType = UnitDebuff(unitID, i) 
                        if debuffType == IsSlowDebuff[x] and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(342938) == 0 and SlowTotal - SlowRemaining >= 0.5 and AuraIsValid(IsSlowDebuff[i].Unit, "UseDispel", "Dispel") and Unit(IsSlowDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then
                            HealingEngine.SetTarget(IsSlowDebuff[i].Unit, 0.5)
                        end
                    end
                    for x = 1, #IsDoTDebuff do                    
                        local name, rank, icon, count, debuffType = UnitDebuff(unitID, i) 
                        if debuffType == IsDoTDebuff[x] and Unit(IsDoTDebuff[i].Unit):HasDeBuffs(342938) == 0 and DoTTotal - DoTRemaining >= 0.5 and AuraIsValid(IsDoTDebuff[i].Unit, "UseDispel", "Dispel") and Unit(IsDoTDebuff[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then
            ...
            if A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if Unit(getmembersAll[i].Unit):HasDeBuffs(350713) > 0 and not Unit(getmembersAll[i].Unit):HasSpec(PVEMELEE) and Unit(getmembersAll[i].Unit):GetRange() <= 40 and not Unit(getmembersAll[i].Unit):IsDead() and AuraIsValid(getmembersAll[i].Unit, "UseDispel", "Dispel") and PVEDispelTotal - PVEDispelRemaining >= 0.5
			...
			if A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if Unit(getmembersAll[i].Unit):HasDeBuffs(350713) > 0 and Unit(getmembersAll[i].Unit):HasSpec(PVEMELEE) and Unit(getmembersAll[i].Unit):GetRange() <= 40 and not Unit(getmembersAll[i].Unit):IsDead() and AuraIsValid(getmembersAll[i].Unit, "UseDispel", "Dispel") and PVEDispelTotal - PVEDispelRemaining >= 0.5
                    and Unit(getmembersAll[i].Unit):HasDeBuffs(342938) == 0 and Unit(getmembersAll[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then  
                        HealingEngine.SetTarget(getmembersAll[i].Unit, 0.5)                                      
                    end                
                end
            ...
            if A.Cleanse:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    if Unit(getmembersAll[i].Unit):HasDeBuffs(350713) == 0 and Unit(getmembersAll[i].Unit):GetRange() <= 40 and not Unit(getmembersAll[i].Unit):IsDead() and AuraIsValid(getmembersAll[i].Unit, "UseDispel", "Dispel") and PVEDispelTotal - PVEDispelRemaining >= 0.5
                    and Unit(getmembersAll[i].Unit):HasDeBuffs(342938) == 0 and Unit(getmembersAll[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then  
                        HealingEngine.SetTarget(getmembersAll[i].Unit, 0.5)                                      
                    end                
                end
            ...
            if A.BlessingofFreedom:IsReady() and A.GetToggle(2, "DispelSniper") then
                for i = 1, #getmembersAll do 
                    ...
                    and AuraIsValid(getmembersAll[i].Unit, true, "BlessingofFreedom") and PVEBOFTotal - PVEBOFRemaining >= 0.5 and Unit(getmembersAll[i].Unit):HasDeBuffs(A.Cyclone.ID) == 0 then  
                        HealingEngine.SetTarget(getmembersAll[i].Unit, 0.5)                                      
                    end                
                end
            end
            ...
            -- #1 HPvE Dispel
            if A.Cleanse:IsReady(unitID) and (((A.Zone ~= "arena" or not A.InstanceInfo.isRated) and PVEDispelTotal - PVEDispelRemaining >= 0.5) or ((A.Zone == "arena" or A.InstanceInfo.isRated) and PVPDispelTotal - PVPDispelRemaining >= 0.5)) and
            ...
            -- #2 HPvE BoF
            if A.BlessingofFreedom:IsReady(unitID) and (((A.Zone == "arena" or A.InstanceInfo.isRated) and PVPBOFTotal - PVPBOFRemaining >= 0.5) or ((A.Zone ~= "arena" or not A.InstanceInfo.isRated) and PVEBOFTotal - PVEBOFRemaining >= 0.5 and Unit(unitID):GetCurrentSpeed() <= 70)) and Unit(unitID):HasBuffs(A.BlessingofFreedom.ID) == 0 and
            useUtils and
            (
                -- MouseOver
                (
        ...
        if A.WordofGlory:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (WordofGloryHP * HealingMultiplier) and Player:HolyPower() >= 5 then
            return A.WordofGlory:Show(icon)
        end
        
        if A.HolyPrism:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (HolyPrismHP * HealingMultiplier) then
            return A.HolyPrism:Show(icon)
        end       
        
        --Holy Shock target
        if A.HolyShock:IsReady(unitID) and IsUnitFriendly(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (HolyShockHP * HealingMultiplier) then
            return A.HolyShock:Show(icon)
        end
        
        --Word of Glory 3 HP
        if A.WordofGlory:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (WordofGloryHP * HealingMultiplier) then
            return A.WordofGlory:Show(icon)
        end
...
TMW:RegisterCallback("TMW_ACTION_HEALINGENGINE_UNIT_UPDATE", function(callbackEvent, thisUnit, db, QueueOrder) 
        ...
        if Unit(player):HasBuffs(A.MaraadBuff.ID) > 0 and MaraadDeficitCheck(1) > 0 then 
            for GUIDs, v in pairs(FriendlyGUIDs) do
                if v.MHP - v.realAHP >= LightofMartyrHP * MaraadMultiplier and Unit(v.Unit):HasBuffs(A.BeaconofFaith.ID, true) == 0 and Unit(v.Unit):HasBuffs(A.BeaconofLight.ID, true) == 0 then
                    v.Enabled = true       
        ...
        if A.HolyShock:IsReady(unitID) and not Unit(unitID):IsDead() and Unit(unitID):HealthPercent() < 90 and combatTime == 0 then
            return A.HolyShock:Show(icon)
        end
        ...
        --Bestow Faith
        if A.BestowFaith:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and Unit(unitID):HasDeBuffs(A.Cyclone.ID) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (BestowFaithHP * HealingMultiplier) then
            return A.BestowFaith:Show(icon)
        end    
        
        if A.DivineFavor:IsTalentLearned() and Unit(player):HasDeBuffs(A.Mindgames.ID) == 0 and IsUnitFriendly("target") and Unit("target"):HasDeBuffs(A.Cyclone.ID) == 0 and Unit("target"):GetRange() <= 40 and Unit("target"):HealthDeficit() >= ((WordofGloryHP/1.211) * HealingMultiplier * 1.5) and A.DivineFavor:IsReady(player) then
        ...
        if A.Zone == "arena" or A.InstanceInfo.isRated or A.Zone == "pvp" or inCombat then
			...
			if Action.GetToggle(2, "AutoTarget") then  
                if (not Unit(target):IsExists() or Unit(target):IsExists() and IsUnitFriendly("target") or (A.Zone ~= "none" and not A.IsInPvP and not Unit(target):IsCracklingShard() and Unit(target):CombatTime() == 0 and Unit(target):IsEnemy() and Unit(target):HealthPercent() >= 100)) 	-- No existed or switch target in PvE if we accidentally selected out of combat unit  			
					and ((not A.IsInPvP and MultiUnits:GetByRangeInCombat(nil, 1) >= 1) or A.Zone == "pvp") and
					(
					(HealingEngine.GetHealthDeficitUnits(WordofGloryHP * HealingMultiplier, 40) == 0 or HealingEngine.GetHealthDeficitUnits(WordofGloryHP * HealingMultiplier, 40) > 0 and not A.WordofGlory:IsReady()) and 
					(HealingEngine.GetHealthDeficitUnits(HolyShockHP * HealingMultiplier, 40) == 0 or HealingEngine.GetHealthDeficitUnits(HolyShockHP * HealingMultiplier, 40) > 0 and not A.HolyShock:IsReady()) and
					(not A.BestowFaith:IsTalentLearned() or (A.BestowFaith:IsTalentLearned() and (HealingEngine.GetHealthDeficitUnits(BestowFaithHP * HealingMultiplier, 40) == 0 or HealingEngine.GetHealthDeficitUnits(BestowFaithHP * HealingMultiplier, 40) > 0 and not A.BestowFaith:IsReady()))) and
					(not A.HolyPrism:IsTalentLearned() or (A.HolyPrism:IsTalentLearned() and (HealingEngine.GetHealthDeficitUnits(HolyPrismHP * HealingMultiplier, 40) == 0 or HealingEngine.GetHealthDeficitUnits(HolyPrismHP * HealingMultiplier, 40) > 0 and not A.HolyPrism:IsReady()))) and
					(HealingEngine.GetHealthDeficitUnits(HolyLightHP * HealingMultiplier, 40) == 0 or Unit(player):GetCurrentSpeed() > 0 or HealingEngine.GetHealthDeficitUnits(HolyLightHP * HealingMultiplier, 40) > 0 and not A.HolyLight:IsReady()) and
					(HealingEngine.GetHealthDeficitUnits(FlashofLightHP * HealingMultiplier, 40) == 0 or Unit(player):GetCurrentSpeed() > 0 or HealingEngine.GetHealthDeficitUnits(FlashofLightHP * HealingMultiplier, 40) > 0 and not A.FlashofLight:IsReady()) and
					(HealingEngine.GetHealthDeficitUnits(LightofMartyrHP * HealingMultiplier, 40) == 0 or HealingEngine.GetHealthDeficitUnits(LightofMartyrHP * HealingMultiplier, 40) > 0 and not A.LightofMartyr:IsReady()) and
					(not A.DivineToll:IsReady() or A.DivineToll:IsReady() and (TeamCacheFriendlyType == "party" and HealingEngine:GetBelowHealthPercentUnits(GetToggle(2, "DivineTollHPParty"), 30) < GetToggle(2, "DivineTollUnitsParty") or (TeamCacheFriendlyType == "raid" and HealingEngine:GetBelowHealthPercentUnits(GetToggle(2, "DivineTollHPRaid"), 30) < GetToggle(2, "DivineTollUnitsRaid")))) and
					(not UseLightofDawn or (UseLightofDawn and (not A.LightofDawn:IsReady() or A.LightofDawn:IsReady() and (HealingEngine.GetBelowHealthPercentUnits(LightofDawnHP, LightofDawnRange) < LightofDawnUnits))))
					)					
    ...
    local function FillerRotation(unitID) 
        ...
        if Unit(player):HasDeBuffs(A.Quake.ID) == 0 then
            ...
            if not inCombat or inCombat and not A.HolyShock:IsReady() and not A.WordofGlory:IsReady() then 
                ...
                if A.HolyLight:IsReady(unitID) and Unit(player):HasDeBuffs(A.Mindgames.ID) < Player:Execute_Time(A.HolyLight.ID) and not A.WordofGlory:IsReady(unitID) and Player:Execute_Time(A.HolyLight.ID) - A.HolyShock:GetCooldown() <= Player:Execute_Time(A.HolyLight.ID)/2 and Unit(player):HasBuffs(A.DivineFavor.ID, true) == 0 and not Unit(unitID):IsDead() and Unit(unitID):HealthDeficit() >= (HolyLightHP * HealingMultiplier) and Unit(player):GetCurrentSpeed() == 0 then
                    return A.HolyLight:Show(icon)
                end    
